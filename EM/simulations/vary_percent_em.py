...
def generate_alpha(tissue, people, precision=10):
    ...
    a = np.zeros((10,  tissue))  # hardcoded the number of people, sorry i know it's bad 
    cols = [0, 0.001, 0.01, 0.1, 1,  5, 10, 20, 50, 100]  # fix tissue # 0 at these percents

    for i in range(a.shape[0]): 
        
        a[i, :] = np.array(generate_proportion_fixed(1, tissue, cols[i], 0))

    return a.repeat(5, axis=0)
...
def generate_em_replicate(i, j, n, depth, gamma_depth): 

    alpha_int = generate_alpha(i, n)  # generate alpha with fixed and varying percents
    ...
    # set there to be an unknown gamma 
    Y_int_depths[-1] = 0 
    Y_int[-1] = 0  
...
if __name__=="__main__": 
    ...
    i = int(sys.argv[3])  # number of tissues, i hardcode this later, so it's set to 10, sorry guys  
    j = int(sys.argv[4])  # number of CpGs
    n = int(sys.argv[5])  # number of people
    depth = int(sys.argv[6])  # input cfDNA depth
    gamma_depth = int(sys.argv[7])  # reference depth

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # run EM 
    Y_depths, Y, X_depths, X, alpha_true, gamma_int = generate_em_replicate(i, j, n, depth, gamma_depth)

    
    # run 10 random restarts and pick the best one 
    random_restarts = []
