...
def complex_mix(data, mix_file): 

    sample = data

    test = sample.iloc[:, 3:25].values.T
    ...
    vals = pkl.load(open(mix_file, "rb" ))
    ...
    return np.nan_to_num(mix_x), np.nan_to_num(mix_x_depth), np.nan_to_num(y_unknown), np.nan_to_num(y_depths_unknown)
...
if __name__=="__main__": 
    ...
    x, x_depths, y, y_depths = complex_mix(data_df, int(sites), str(mix_file))
