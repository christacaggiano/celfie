...
def generate_replicate(i, j, n, depth, gamma_depth, pkl_file, rep_num): 
    ...
    # to run it for 
    alpha_int = generate_alpha(kl_file)[rep_num].reshape(1, -1)  # reshape to be compatible dimensions
...
def least_squares_regression(X, X_depths, Y, Y_depths): 
    ...
    return np.linalg.lstsq(np.nan_to_num(y_proportions), np.nan_to_num(x_proportions.flatten()), rcond=None)
...
if __name__ == "__main__": 
    ...
    Y_depths, Y, X_depths, X, alpha_true, gamma_int = generate_replicate(i, j, n, depth, gamma_depth, pkl_file, rep_num)
